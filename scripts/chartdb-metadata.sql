/*
 * ChartDB metadata query for PostgreSQL.
 *
 * Copied verbatim from ChartDB v1.20.1, src/lib/data/import-metadata/scripts/
 * postgres-script.ts, evaluated for the plain PostgreSQL edition (the upstream file is a
 * TypeScript builder whose Supabase/Timescale branches resolve to empty strings here).
 *
 * Kept as its own file, not inlined in genCDB.js, so it can be diffed against upstream
 * when ChartDB changes its import format. Do not hand-edit: re-extract instead.
 */
/* PostgreSQL edition */
WITH fk_info AS (
